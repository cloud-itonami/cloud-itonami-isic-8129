(ns industrialcleaningops.governor-contract-test
  ...
  (testing "clean service-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-service-record :site-id "site-1" :patch {:areas-cleaned 12}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-safety-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-safety-concern :site-id "site-1"
                                :patch {:concern "confined-space entry planned, low-oxygen reading" :confidence 0.99}}
                               ctx)]
      (is (some? result))
  ...
  (testing "a high-cost :coordinate-supply-order escalates for human approval, even at phase 3 clean"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :site-id "site-1"
                                :patch {:item "confined-space ventilation rig lease" :quantity 1 :estimated-cost 3600.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
  ...
  (testing "unregistered site -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t3"
                     {:op :log-service-record :site-id "unknown-site"
                      :patch {:areas-cleaned 0}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
  ...
  (testing "registered but unverified site -> permanent HARD hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t4"
                               {:op :log-service-record :site-id "site-3"
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-service-record :site-id "site-1"
                                :patch {:areas-cleaned 12}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into hazmat-handling-safety-clearance/confined-space-entry-authorization finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-service-record :site-id "unknown" :patch {:areas-cleaned 12}}
                     ctx)
