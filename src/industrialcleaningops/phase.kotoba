(ns industrialcleaningops.phase
  "Phase 0->3 staged rollout for the ISIC-8129 'Other building and
  industrial cleaning activities' operations-coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- service-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds cleaning-crew/equipment
                                       dispatch scheduling and supply-
  ...
  rollout milestone still to come. Flagging a hazmat/confined-space/
  chemical-exposure safety concern always needs a human to actually look
  at it; this actor never directly finalizes a hazmat-handling-safety
  clearance or a confined-space-entry authorization itself.
  `industrialcleaningops.governor`'s own `always-escalate-ops` enforces
  the same invariant independently -- two layers, not one, agree on
  this."
  (:require [industrialcleaningops.governor :as governor]))
...
(defn verdict->disposition
  "Map an IndustrialCleaningOpsGovernor verdict to a base disposition
