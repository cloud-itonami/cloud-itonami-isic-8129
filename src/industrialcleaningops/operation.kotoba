(ns industrialcleaningops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (IndustrialCleaningOpsAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the IndustrialCleaningOpsGovernor
  ...
  human cleaning-operations coordinator/hazmat-safety coordinator. The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [industrialcleaningops.advisor :as advisor]
            [industrialcleaningops.governor :as governor]
            [industrialcleaningops.phase :as phase]
...
(defn build
  ...
  `industrialcleaningops.store/Store`).
  opts:
    :advisor      -- an `industrialcleaningops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; IndustrialCleaningOpsAdvisor inference (the contained
      ...
      ;; IndustrialCleaningOpsGovernor -- independent censor (separate
      ;; system than the advisor).
      (g/add-node :govern
